...
Object.defineProperty(exports, "JsonSnapsRegistry", {
    enumerable: true,
    get: function() {
        return JsonSnapsRegistry;
    }
});
const _basecontroller = require("@metamask/base-controller");
const _snapsregistry = require("@metamask/snaps-registry");
...
class JsonSnapsRegistry extends _basecontroller.BaseController {
    ...
    }, publicKey, fetchFunction = globalThis.fetch.bind(globalThis), recentFetchThreshold = (0, _utils.inMilliseconds)(5, _utils.Duration.Minute), failOnUnavailableRegistry = true, refetchOnAllowlistMiss = true }){
        super({
            messenger,
            metadata: {
                database: {
...
async function getSingle(snapId, snapInfo, refetch = false) {
    ...
    const blockedEntry = database?.blockedSnaps.find((blocked)=>{
        if ('id' in blocked) {
            return blocked.id === snapId && (0, _utils.satisfiesVersionRange)(snapInfo.version, blocked.versionRange);
        }
        return blocked.checksum === snapInfo.checksum;
    });
    if (blockedEntry) {
        return {
            status: _registry.SnapsRegistryStatus.Blocked,
    ...
    if (version && version.checksum === snapInfo.checksum) {
        return {
            status: _registry.SnapsRegistryStatus.Verified
        };
    }
    ...
    return {
        status: _registry.SnapsRegistryStatus.Unverified
    };
}
...
function verifySignature(database, signature) {
    ...
    (0, _utils.assert)(valid, 'Invalid registry signature.');
}
