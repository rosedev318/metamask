...
var PermissionController = class extends _basecontroller.BaseController {
  ...
  constructor(options) {
    ...
    this.createPermissionMiddleware = _chunkAQ35E2HUjs.getPermissionMiddlewareFactory.call(void 0, {
  ...
  validatePermissionSpecifications(permissionSpecifications, caveatSpecifications) {
    Object.entries(
      ...
      ([
        ...
        if (!permissionType || !_utils.hasProperty.call(void 0, _chunk6CID6TBWjs.PermissionType, permissionType)) {
        ...
        if (allowedCaveats) {
          allowedCaveats.forEach((caveatType) => {
            if (!_utils.hasProperty.call(void 0, caveatSpecifications, caveatType)) {
              throw new (0, _chunkU5LTEXSUjs.UnrecognizedCaveatTypeError)(caveatType);
            }
            const specification = caveatSpecifications[caveatType];
            const isRestrictedMethodCaveat = _chunkEGNDXGRGjs.isRestrictedMethodCaveatSpecification.call(void 0, specification);
            if (permissionType === "RestrictedMethod" /* RestrictedMethod */ && !isRestrictedMethodCaveat || permissionType === "Endowment" /* Endowment */ && isRestrictedMethodCaveat) {
              throw new (0, _chunkU5LTEXSUjs.CaveatSpecificationMismatchError)(
                specification,
                permissionType
              );
            }
  ...
  getTypedPermissionSpecification(permissionType, targetName, requestingOrigin) {
    const failureError = permissionType === "RestrictedMethod" /* RestrictedMethod */ ? _chunkU5LTEXSUjs.methodNotFound.call(void 0, 
      targetName,
      requestingOrigin ? { origin: requestingOrigin } : void 0
    ) : new (0, _chunkU5LTEXSUjs.EndowmentPermissionDoesNotExistError)(
      targetName,
      requestingOrigin
    );
    ...
    if (!_chunk6CID6TBWjs.hasSpecificationType.call(void 0, specification, permissionType)) {
      throw failureError;
    }
  ...
  revokeAllPermissions(origin) {
    this.update((draftState) => {
      if (!draftState.subjects[origin]) {
        throw new (0, _chunkU5LTEXSUjs.UnrecognizedSubjectError)(origin);
      }
      ...
      Object.keys(subjectsAndPermissions).forEach((origin) => {
        ...
        subjectsAndPermissions[origin].forEach((target) => {
          ...
          if (!_utils.hasProperty.call(void 0, permissions, target)) {
            throw new (0, _chunkU5LTEXSUjs.PermissionDoesNotExistError)(origin, target);
          }
  ...
  getCaveat(origin, target, caveatType) {
    ...
    return _chunk6CID6TBWjs.findCaveat.call(void 0, permission, caveatType);
  }
  /**
  ...
  addCaveat(origin, target, caveatType, caveatValue) {
    if (this.hasCaveat(origin, target, caveatType)) {
      throw new (0, _chunkU5LTEXSUjs.CaveatAlreadyExistsError)(origin, target, caveatType);
    }
  ...
  updateCaveat(origin, target, caveatType, caveatValue) {
    if (!this.hasCaveat(origin, target, caveatType)) {
      throw new (0, _chunkU5LTEXSUjs.CaveatDoesNotExistError)(origin, target, caveatType);
    }
    this.setCaveat(origin, target, caveatType, caveatValue);
  }
  /**
  ...
  deleteCaveat(permission, caveatType, origin) {
    if (!permission.caveats) {
      throw new (0, _chunkU5LTEXSUjs.CaveatDoesNotExistError)(
        origin,
        permission.parentCapability,
        caveatType
      );
  ...
  grantPermissions({
    ...
    if (!origin || typeof origin !== "string") {
      throw new (0, _chunkU5LTEXSUjs.InvalidSubjectIdentifierError)(origin);
    }
    ...
    for (const [requestedTarget, approvedPermission] of Object.entries(
      ...
      if (!this.targetExists(requestedTarget)) {
        throw _chunkU5LTEXSUjs.methodNotFound.call(void 0, requestedTarget);
      }
      if (approvedPermission.parentCapability !== void 0 && requestedTarget !== approvedPermission.parentCapability) {
        throw new (0, _chunkU5LTEXSUjs.InvalidApprovedPermissionError)(
          origin,
          requestedTarget,
          approvedPermission
        );
      ...
      if (specification.factory) {
        ...
        permission = _chunk6CID6TBWjs.constructPermission.call(void 0, permissionOptions);
        this.validatePermission(specification, permission, origin, {
  ...
  validatePermission(specification, permission, origin, { invokePermissionValidator, performCaveatValidation } = {
    ...
    if (specification.subjectTypes?.length && specification.subjectTypes.length > 0) {
      ...
      if (!metadata || metadata.subjectType === null || !specification.subjectTypes.includes(metadata.subjectType)) {
        throw specification.permissionType === "RestrictedMethod" /* RestrictedMethod */ ? _chunkU5LTEXSUjs.methodNotFound.call(void 0, targetName, { origin }) : new (0, _chunkU5LTEXSUjs.EndowmentPermissionDoesNotExistError)(targetName, origin);
      }
    }
    if (_utils.hasProperty.call(void 0, permission, "caveats")) {
      const { caveats } = permission;
      if (caveats !== null && !(Array.isArray(caveats) && caveats.length > 0)) {
        throw new (0, _chunkU5LTEXSUjs.InvalidCaveatsPropertyError)(origin, targetName, caveats);
      }
      ...
      caveats?.forEach((caveat) => {
        ...
        if (!allowedCaveats?.includes(caveat.type)) {
          throw new (0, _chunkU5LTEXSUjs.ForbiddenCaveatError)(caveat.type, origin, targetName);
        }
        if (seenCaveatTypes.has(caveat.type)) {
          throw new (0, _chunkU5LTEXSUjs.DuplicateCaveatError)(caveat.type, origin, targetName);
        }
  ...
  validateCaveat(caveat, origin, target) {
    if (!_controllerutils.isPlainObject.call(void 0, caveat)) {
      throw new (0, _chunkU5LTEXSUjs.InvalidCaveatError)(caveat, origin, target);
    }
    if (Object.keys(caveat).length !== 2) {
      throw new (0, _chunkU5LTEXSUjs.InvalidCaveatFieldsError)(caveat, origin, target);
    }
    if (typeof caveat.type !== "string") {
      throw new (0, _chunkU5LTEXSUjs.InvalidCaveatTypeError)(caveat, origin, target);
    }
    const specification = this.getCaveatSpecification(caveat.type);
    if (!specification) {
      throw new (0, _chunkU5LTEXSUjs.UnrecognizedCaveatTypeError)(caveat.type, origin, target);
    }
    if (!_utils.hasProperty.call(void 0, caveat, "value") || caveat.value === void 0) {
      throw new (0, _chunkU5LTEXSUjs.CaveatMissingValueError)(caveat, origin, target);
    }
    if (!_controllerutils.isValidJson.call(void 0, caveat.value)) {
      throw new (0, _chunkU5LTEXSUjs.CaveatInvalidJsonError)(caveat, origin, target);
    }
  ...
  validateRequestedPermissions(origin, requestedPermissions) {
    if (!_controllerutils.isPlainObject.call(void 0, requestedPermissions)) {
      throw _chunkU5LTEXSUjs.invalidParams.call(void 0, {
    ...
    for (const targetName of Object.keys(requestedPermissions)) {
      ...
      if (!this.targetExists(targetName)) {
        throw _chunkU5LTEXSUjs.methodNotFound.call(void 0, targetName, { origin, requestedPermissions });
      }
    ...
    if (rejectedHandlers.length > 0) {
      ...
      if (failureHandlersList.length > 0) {
        try {
          ...
          throw _chunkU5LTEXSUjs.internalError.call(void 0, "Unexpected error in side-effects", { error });
        }
      }
      ...
      throw reasons.length > 1 ? _chunkU5LTEXSUjs.internalError.call(void 0, 
  ...
  validateApprovedPermissions(approvedRequest, originalMetadata) {
    ...
    if (!_controllerutils.isPlainObject.call(void 0, approvedRequest) || !_controllerutils.isPlainObject.call(void 0, approvedRequest.metadata)) {
      throw _chunkU5LTEXSUjs.internalError.call(void 0, 
    ...
    try {
      ...
      if (error instanceof _rpcerrors.JsonRpcError) {
        throw _chunkU5LTEXSUjs.internalError.call(void 0, 
          `Invalid approved permissions request: ${error.message}`,
          error.data
        );
      }
      throw _chunkU5LTEXSUjs.internalError.call(void 0, "Unrecognized error type", { error });
    }
  }
  /**
  ...
  async acceptPermissionsRequest(request) {
    ...
    if (!this.hasApprovalRequest({ id })) {
      throw new (0, _chunkU5LTEXSUjs.PermissionsRequestNotFoundError)(id);
    }
    if (Object.keys(request.permissions).length === 0) {
      this._rejectPermissionsRequest(
        id,
        _chunkU5LTEXSUjs.invalidParams.call(void 0, {
  ...
  async rejectPermissionsRequest(id) {
    ...
    this._rejectPermissionsRequest(id, _chunkU5LTEXSUjs.userRejectedRequest.call(void 0, ));
  }
  /**
  ...
  async getEndowments(origin, targetName, requestData) {
    if (!this.hasPermission(origin, targetName)) {
      throw _chunkU5LTEXSUjs.unauthorized.call(void 0, { data: { origin, targetName } });
    }
    return this.getTypedPermissionSpecification(
      "Endowment" /* Endowment */,
      targetName,
  ...
  _executeRestrictedMethod(methodImplementation, subject, method, params = []) {
    ...
    if (!permission) {
      throw _chunkU5LTEXSUjs.unauthorized.call(void 0, { data: { origin, method } });
    }
    return _chunkEGNDXGRGjs.decorateWithCaveats.call(void 0, 
      methodImplementation,
      permission,
      this._caveatSpecifications
