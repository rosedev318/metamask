...
class AbstractExecutionService {
   ...
   */ async initJob() {
        ...
        (0, _readablestream.pipeline)(jsonRpcConnection.stream, streams.command, jsonRpcConnection.stream, (error)=>{
            if (error) {
   ...
   */ async executeSnap(snapData) {
    ...
    async command(jobId, message) {
        (0, _utils.assertIsJsonRpcRequest)(message);
        ...
        if (response.error) {
            throw new _rpcerrors.JsonRpcError(response.error.code, response.error.message, response.error.data);
        }
...
function setupMultiplex(connectionStream, streamName) {
    ...
    (0, _readablestream.pipeline)(connectionStream, mux, connectionStream, (error)=>{
        if (error) {
