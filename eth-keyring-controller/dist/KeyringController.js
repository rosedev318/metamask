...
class KeyringController extends events_1.EventEmitter {
    constructor({ keyringBuilders, cacheEncryptionKey, initState = {}, encryptor = encryptorUtils, }) {
        ...
        this.store = new obs_store_1.ObservableStore(initState);
     ...
     */
    async prepareUserOperation(rawAddress, transactions) {
        const address = (0, eth_sig_util_1.normalize)(rawAddress);
        const keyring = await this.getKeyringForAccount(address);
        return keyring.prepareUserOperation
            ? await keyring.prepareUserOperation(address, transactions)
            : (0, utils_2.throwError)(constants_1.KeyringControllerError.UnsupportedPrepareUserOperation);
    }
    /**
     ...
     */
    async patchUserOperation(rawAddress, userOp) {
        const address = (0, eth_sig_util_1.normalize)(rawAddress);
        const keyring = await this.getKeyringForAccount(address);
        return keyring.patchUserOperation
            ? await keyring.patchUserOperation(address, userOp)
            : (0, utils_2.throwError)(constants_1.KeyringControllerError.UnsupportedPatchUserOperation);
    }
    /**
     ...
     */
    async signUserOperation(rawAddress, userOp) {
        const address = (0, eth_sig_util_1.normalize)(rawAddress);
        const keyring = await this.getKeyringForAccount(address);
        return keyring.signUserOperation
            ? await keyring.signUserOperation(address, userOp)
            : (0, utils_2.throwError)(constants_1.KeyringControllerError.UnsupportedSignUserOperation);
    }
    /**
     * =========================================
    ...
    async addNewKeyring(type, opts) {
        ...
        if (type === constants_1.KeyringType.HD && (!(0, utils_1.isObject)(opts) || !opts.mnemonic)) {
...
async function _KeyringController_destroyKeyring(keyring) {
    var _a;
    await ((_a = keyring.destroy) === null || _a === void 0 ? void 0 : _a.call(keyring));
