...
var web3Utils = require('web3').utils; // eslint-disable-line
...
function testRandomValueAgainstWeb3ToWei(negative) {
  ...
  var web3Value = new BigNumber(web3Utils.toWei(stringTestValue, randomunitsType));
...
function testRandomValueAgainstWeb3FromWei(negative) {
  ...
  var web3Value = web3Utils.fromWei(stringTestValue, randomunitsType);
...
describe('fromWei', function () {
  it('should handle options', function () {
    assert.equal(units.fromWei(10000000, 'wei', {
      commify: true
    }), '10,000,000');
  });
  it('should return the correct value', function () {
